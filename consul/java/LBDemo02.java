...
public class LBDemo02 {
   
    private static long connectionCount=0;
    private static long runSimple=0;
	...
	System.out.println("Getting New Connection [" + ++connectionCount + "]");
	runSimple=0;
    ...
    static void executeSimpleTransaction(Connection c, int conn, int trans){
        try {
	    System.out.println("Running Simple Transaction [ " + ++runSimple + "]");
	    c.setReadOnly(false);
            c.setAutoCommit(false);
            Statement s = c.createStatement();
