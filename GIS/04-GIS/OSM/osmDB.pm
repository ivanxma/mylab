...
sub storeDBNode {
	...
	$dbh->do("INSERT INTO nodes (id, geom, user, version, timestamp, uid, changeset) VALUES ($id, POINT($lon, $lat), '$user', $version, '$timestamp', $uid, $changeset)") ;
...
sub dbConnect {
	...
	$dbh = DBI->connect("DBI:mysql:$DBname;host=127.0.0.1;port=3306", $DBuser, $DBpassword) or die ("error connecting DB: $DBI::errstr\n") ;
...
sub initTableNodes {
	...
	$dbh->do("create table nodes ( id BIGINT, geom GEOMETRY NOT NULL, user VARCHAR(50), version INTEGER, timestamp VARCHAR(20), uid INTEGER, changeset INTEGER )") ;	
	$dbh->do("CREATE SPATIAL INDEX i_geomidx ON nodes (geom)") ;	
...
sub bulkLoad {
	...
	$dbh->do("LOAD DATA LOCAL INFILE '$tempDir/nodes.txt' INTO TABLE nodes (id, \@lon, \@lat, user, version, timestamp, uid, changeset) set geom = POINT(\@lon, \@lat)") ;
